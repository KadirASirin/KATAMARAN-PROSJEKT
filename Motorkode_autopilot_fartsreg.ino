...
//---RC KONTROLLER--------------------------------------------------------------------------
  ...
  styring = map(receiver.getRaw(1), 1050, 2050, -150, 150);
  gass = map(receiver.getRaw(2), 1010, 2020, -200, 200);
  int new_mode = map(receiver.getRaw(3), 1100, 1900, 0, 1);

  // Rc verdier kun ved to like verdier for å unngå feilbeskjed
  /*
  if (new_styring == prev_styring) {
    styring = new_styring; 
  }
  prev_styring = new_styring;

  if (new_gass == prev_gass) {
    gass = new_gass; 
  }
  prev_gass = new_gass;
  */
  ...
  //Serial.println(receiver.getRaw(3));
...
//---STYRING VIA REGULERING OG PYTHON INPUTS FRA SERIELL--------------------------------------------------------------------------
    ...
    while (mySerial.available() > 0) {
      ...
      // Check if the received string contains three commas
      int commaCount = 0;
      ...
      // If there are not three commas, skip parsing
      if (commaCount != 3) {
        //Serial.println("Invalid format: Missing value(s)");
        continue;
      }
      ...
      if (commaIndex1 != -1 && commaIndex2 != -1 && commaIndex3 != -1) {
        ...
        // Extract substring before the first comma and convert to float
        target_speed = receivedString.substring(0, commaIndex1).toFloat();
        target_speed = constrain(target_speed, -2, 7);

        // Extract substring between first and second comma and convert to float
        target_angle = receivedString.substring(commaIndex1 + 1, commaIndex2).toFloat();
        target_angle = constrain(target_angle, 0, 360);
    
        // Extract substring between second and third comma and convert to float
        float new_current_speed = receivedString.substring(commaIndex2 + 1, commaIndex3).toFloat();
      
        // Extract substring after the third comma and convert to float
        float new_current_angle = receivedString.substring(commaIndex3 + 1).toFloat();

        // Compare with previous values and save if the change is over 1.0
    ...
    if (current_speed < abs(target_speed)) {
      // Sjekker timer
      if (f_timer >= dt && f_timer != 0) {
        
        f_error = abs(target_speed - current_speed);

        // integrasjon trapes regel
        f_integral += 0.5 * (f_error + f_prev_error) * f_timer;

        // PI utregning
        f_reg = f_Kp * f_error + f_Ki * f_integral;
    ...
    //else if (current_speed > abs(target_speed)) {
      //MotorSpeed1 = MotorSpeed1 - f_reg;
      //MotorSpeed2 = MotorSpeed2 - f_reg;
    //}
    else{
    ...
    // Ensure the distance is between -180 and 180 degrees
    if (v_avstand > 180) {
      v_avstand -= 360; // Convert to negative equivalent

    }
    else if (v_avstand < -180) {
      v_avstand += 360; // Convert to positive equivalent
    }
    ...
    if (v_avstand > 0) {
      ...
      a_retning = 0; // No need to turn, already at target angle
    }
    ...
    // Check if target_angle has changed
    if (target_angle != prev_target_angle) {
      // Reset the integral term to 0
      a_integral = 0;
      // Update prev_target_angle to the current target_angle
    ...
    if (current_angle != target_angle) {
      ...
      if (a_timer >= dt && a_timer != 0) {
        ...
        // PI utregning
        a_reg = a_Kp * a_error + a_Ki * a_integral;
